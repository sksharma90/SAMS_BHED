...
public class ModificationSaveAsDraftFunctionality extends BaseClass implements BHEdData {
	...
	@Test(priority = 1)
	public void SuccessfulModificationAfterSaveAsDraft() throws InterruptedException {
		...
		ExtentFactory.getInstance().getExtent().info("Modified the Roll No");
		Form.TenthRollNo().sendKeys(TenthRollNumberM);
		Thread.sleep(3000);
		...
		Thread.sleep(1000);
		ExtentFactory.getInstance().getExtent().info("Modified Hindi Qualification");
		...
		Thread.sleep(1000);
		ExtentFactory.getInstance().getExtent().info("Modified Pincode");
		Form.PresentPinCode().sendKeys(PincodeM);
		Thread.sleep(1000);
		...
		if (!(RoNumber.equals(TenthRollNumberM) && Aname.equals(ApplicantName) && Fname.equals(FatherName)
				&& MName.equals(MotherName) && Email.equals(Email) && Aadhar.equals(AadhaarNo) && PAdd1.equals(Add1)
				&& PAdd2.equals(Add2) && PPin.equals(PincodeM) && TwelthRoll.equals(TwelthRollNumber)
		...
		softAssert.assertEquals(true, RoNumber.equals(TenthRollNumberM) && Aname.equals(ApplicantName)
				&& Fname.equals(FatherName) && MName.equals(MotherName) && Email.equals(Email)
				&& Aadhar.equals(AadhaarNo) && PAdd1.equals(Add1) && PAdd2.equals(Add2) && PPin.equals(PincodeM)
				&& TwelthRoll.equals(TwelthRollNumber) && TwelthMax.equals(TwelthMaxMark)
				&& TwelthSec.equals(TwelthSecuredMark) && GradRoll.equals(GradRollNumber) && GradMax.equals(GradMaxMark)
				&& GradSec.equals(GradSecuredMark) && PostGradRoll.equals(PGRollNumber) && PostGradMax.equals(PGMaxMark)
				&& PostGradSec.equals(PGSecuredMark));

		ExtentFactory.getInstance().getExtent()
				.info("Successfully Checked Successful Modification After Save As Draft scenario");
