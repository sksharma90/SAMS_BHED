...
public class BacktoModifyfunctionality extends BaseClass implements BHEdData {
...
//		bc.Login("7978329584", Password);
	...
	public void BackToModify() throws Exception {
		...
		Form.JavaAcceptAlert();
		Thread.sleep(1000);
		Form.PGMaxMark().sendKeys(GradMaxMark);
		Thread.sleep(1000);
		Form.PGObtainedMark().clear();
		Thread.sleep(1000);
		Form.PGObtainedMark().sendKeys(GradSecuredMark);
		Thread.sleep(1000);
		...
		if (AppName.equals(ApplicantName) && FName.equals(FatherName) && MName.equals(MotherName)
			...
			System.out.println(AppName + Aadhar + Pin + TwelthRollNumberr + TwlthMin + Data2 + Data6 + Data11 + FName
					+ Address1 + Address2 + MName + RollNumber + TenthMax + TenthMin + TwelthMax + Data + Data1 + Data3
					+ Data4 + Data5 + Data9 + Data7 + Data12 + Data10);
	...
	public void DuplicateRollNumberCheckBackToModify() throws Exception {
		...
		ExtentFactory.getInstance().getExtent()
				.info("Successfully Checked Acceptance of Duplicate Roll Number during Back To Modify Scenario");
